...
from config import SERVER, SSID, WIFI_PASSWORD, USER, PASSWORD, COUNTER,REAL_COUNTER, init_config, setCounter
...
MAX_BATTERY = const(3.7)  # 4 x 1.3v - 4xAA NiCM
MIN_BATTERY = const(3)  
...
def calculateMeter (meter):
    global COUNTER, REAL_COUNTER
    counterInt = int(COUNTER)
    realCounterInt = int(REAL_COUNTER)
    diff = meter - counterInt
    if (diff > 0 & diff < 20):
        REAL_COUNTER = str(realCounterInt + diff)
    COUNTER = str(meter)
